...
run <- file.path(root, "05_analysis_steps/M02_COHORT_QC/runs/20260814_M02_B4_GSE232306")
out <- file.path(run, "results/expression_qc")

pca <- read.csv(file.path(out, "GSE232306_PCA_COORDINATES.csv"), check.names = FALSE)
cols <- c(DOR = "#D55E00", NOR = "#0072B2")
png(file.path(out, "GSE232306_PCA.png"), width = 2200, height = 1600, res = 240)
...
plot(pca$PC1, pca$PC2, pch = 21, bg = cols[pca$phenotype], col = "white", cex = 2.0,
     ...
     main = "GSE232306: gene-level expression PCA")
label_pos <- c(3, 3, 2, 3, 3, 2, 3, 3, 3, 3, 4, 1)
text(pca$PC1, pca$PC2, labels = pca$srr, pos = label_pos, cex = 0.66)
legend("topright", inset = c(-0.16, 0), legend = names(cols), pt.bg = cols, pch = 21, bty = "n")
dev.off()

corr <- as.matrix(read.csv(file.path(out, "GSE232306_SAMPLE_CORRELATION.csv"), row.names = 1, check.names = FALSE))
png(file.path(out, "GSE232306_SAMPLE_CORRELATION.png"), width = 2100, height = 1900, res = 240)
par(mar = c(9, 9, 3, 2))
image(seq_len(nrow(corr)), seq_len(ncol(corr)), t(corr[nrow(corr):1, ]),
      ...
      main = "GSE232306 sample correlation")
...
markers <- read.csv(file.path(out, "GSE232306_MARKER_EXPRESSION.csv"), check.names = FALSE)
...
png(file.path(out, "GSE232306_MARKER_EXPRESSION.png"), width = 2200, height = 2400, res = 240)
par(mar = c(9, 9, 3, 2))
...
cat("PASS: GSE232306 PCA, correlation and marker QC plots generated\n")
