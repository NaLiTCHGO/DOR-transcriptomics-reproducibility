...
if len(cfg) != 12 or cfg["srr"].nunique() != 12:
    raise RuntimeError("The frozen GSE232306 sample table must contain 12 unique samples")
...
pd.concat([meta, tpm], axis=1).reset_index().to_csv(
    OUT / "GSE232306_GENE_TPM.tsv.gz", sep="\t", index=False, compression="gzip"
)
pd.concat([meta, counts], axis=1).reset_index().to_csv(
    OUT / "GSE232306_GENE_ESTIMATED_COUNTS.tsv.gz", sep="\t", index=False, compression="gzip"
)
...
pd.DataFrame(corr_qc).to_csv(OUT / "GSE232306_SAMPLE_CORRELATION_QC.csv", index=False)
...
markers.to_csv(OUT / "GSE232306_MARKER_EXPRESSION.csv", index=False)
...
pd.DataFrame(sample_marker).to_csv(OUT / "GSE232306_MARKER_QC_BY_SAMPLE.csv", index=False)

summary = f"""# GSE232306 Expression QC Summary
...
- Individual age values: not available in the public GEO sample metadata; age cannot be used in the within-cohort design.
- Interpretation boundary: PCA and marker panels are QC evidence, not differential-expression findings.
"""
(OUT / "GSE232306_EXPRESSION_QC_SUMMARY.md").write_text(summary, encoding="utf-8")
(RUN / "STEP05_EXPRESSION_QC.PASS.txt").write_text("PASS\tgene matrices and numerical QC generated\n", encoding="utf-8")
print(f"PASS: GSE232306 expression matrices and numerical QC generated for {len(sample_order)} samples")
